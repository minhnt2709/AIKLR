...
class SemiCrossEncoder:
    def __init__(
        ...
        self.model = BertExtendModel.from_pretrained(
        ...
        self.cross_attention = MulitAttentionLayer(
            cross_attention_heads, self.config.hidden_size
        )
    ...
    def transform_graph_geometric(self, embeddings, edge_index, edge_type):
        list_geometric_data = [
            Data(
                x=emb,
                edge_index=torch.tensor(edge_index[idx], dtype=torch.long),
                y=torch.tensor(edge_type[idx], dtype=torch.long),
            )
    ...
    def fit(
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for features, labels in tqdm(
                ...
                if edge_index_a:
                    graph_batch_a = self.transform_graph_geometric(
                        feature_a["input_ids"], edge_index_a, edge_type_a
                    )
                    feature_a["graph_batch"] = graph_batch_a
                    graph_batch_b = self.transform_graph_geometric(
                        feature_b["input_ids"], edge_index_b, edge_type_b
                    )
                ...
                if use_amp:
                    with autocast():
                        ...
                        biatt_encoded_a, ab_att = self.cross_attention(
                            model_encoded_a,
                            model_encoded_b,
                            feature_a["attention_mask"],
                        )  # b is query
                        biatt_encoded_b, ba_att = self.cross_attention(
                            model_encoded_b,
                            model_encoded_a,
                            feature_b["attention_mask"],
                        )  # a is query
                        # output_cross_ab = torch.mean(
                        #     torch.stack([torch.mean(biatt_encoded_a, 1), torch.mean(biatt_encoded_b, 1)]),
                        #     dim=0)
                        biatt_encoded_a, biatt_encoded_b = (
                            biatt_encoded_a[:, 0, :],
                            biatt_encoded_b[:, 0, :],
                        )
                        logits = activation_fct(
                            torch.cosine_similarity(biatt_encoded_a, biatt_encoded_b)
                        )
                    ...
                    model_encoded_b = self.model(**feature_b, return_dict=True)[0]
                    biatt_encoded_a, ab_att = self.cross_attention(
                        model_encoded_a, model_encoded_b, feature_a["attention_mask"]
                    )  # b is query
                    biatt_encoded_b, ba_att = self.cross_attention(
                        model_encoded_b, model_encoded_a, feature_b["attention_mask"]
                    )  # a is query
                    # output_cross_ab = torch.mean(
                    #     torch.stack([torch.mean(biatt_encoded_a, 1), torch.mean(biatt_encoded_b, 1)]),
                    #     dim=0)
...
class MulitAttentionLayer(nn.Module):
    ...
    def __init__(self, n_head, hidden_size, dropout=0.1):
        super().__init__()
        assert (
            hidden_size % n_head == 0
        )  # make sure the output dimension equals to hidden_size
        self.n_head = n_head
    ...
    def forward(self, context_states, query_states, attention_mask):
        ...
        context_layer = context_layer.permute(
            0, 2, 1, 3
        ).contiguous()  # [b, seq, n_head, head_hs]
